...
public class Main {
	...
	static int length(char car) {
		return isType(car, LONGS) ? 3 : isType(car, SHORTS) ? 2 : 0 / 0; // a
																			// nasty
																			// shortcut
																			// for
																			// throwing
	...
	// in given state, returns the entity at a given coordinate, possibly out of
	// bound
	...
	// in a given state, starting from given coordinate, toward the given
	// direction,
	...
	// really long shortest-path trace (which is infeasible in standard Rush
	// Hour)
	...
	// in a given state, from a given origin coordinate, attempts to find a car
	// of a given type
	// at a given distance in a given direction; if found, slide it in the
	// opposite direction
	// one spot at a time, exactly n times, proposing those states to the
	...
	// X nR = 4
	//
	// The n? counts how many spaces are there in a given direction, origin
	// inclusive.
